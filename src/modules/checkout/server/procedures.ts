...
export const checkoutRouter = createTRPCRouter({
    verify: protectedProcedure.mutation(async ({ ctx }) => {
        const user = await ctx.db.findByID({
            ...
            depth: 0, // user.tenants[0].tenant is going to be a string
        });
        ...
        if (!accountLink) throw new TRPCError({
            code: 'BAD_REQUEST',
    ...
    purchase: protectedProcedure.input(
        ...
        if (!tenant.stripeDetailSubmitted) throw new TRPCError({
            code: 'BAD_REQUEST',
            message: 'Tenant not allowed to sell product'
        });

        // percentage cut for the super-admin... me, respectfully
        ...
        const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = products.docs.map((product) => ({
            quantity: 1,
            price_data: {
                unit_amount: Math.round((product.price / 122) * 100), // Stripe expects cents and we convert to british pounds
                currency: 'eur' ,
                product_data: {
                    name: product.name,
                    metadata: {
