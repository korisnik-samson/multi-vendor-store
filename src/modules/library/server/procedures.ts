...
export const libraryRouter = createTRPCRouter({
    getOne: protectedProcedure.input(
        ...
        const ordersData = await ctx.db.find({
            collection: 'orders',
            depth: 0, // we want to get just the IDs without populating
            pagination: false,
            limit: 1,
            where: {
                and: [
        ...
        if (!order) throw new TRPCError({
            code: 'NOT_FOUND',
            message: 'Order not found in your library'
        });
    ...
    getMany: protectedProcedure.input(
        ...
        const dataWithSummarisedReviews = await Promise.all(productsData.docs.map(async (doc) => {
